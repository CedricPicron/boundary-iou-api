...
# set up iou type
args.iou_type = os.environ.get('IOU_TYPE', 'segm')
args.dilation_ratio = os.environ.get('DILATION_RATIO', 0.005)
...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    if args.iou_type == "boundary":
        gtBoundary = gtNp.copy()
        # largest label id in CS is bicycle: 33
        BOUNDARY_ID = 34000
        for label in gtInstances:
            gtInstancesNew = []
            for gtInstance in gtInstances[label]:
                l = gtInstance["instID"]
                binary_mask = (gtBoundary == l).astype(np.uint8)
                binary_boundary = mask_to_boundary(binary_mask, args.dilation_ratio)
                gtBoundary[binary_mask > 0] = BOUNDARY_ID
                gtBoundary[binary_boundary > 0] = l
                gtInstance["pixelCountBoundary"] = int((gtBoundary == l).sum())
                gtInstancesNew.append(gtInstance)
            gtInstances[label] = gtInstancesNew
    else:
        gtBoundary = None
    ...
    for predImageFile in predInfo:
        ...
        # Get pred boundary
        if args.iou_type == "boundary":
            predBoundary = predNp.copy()
            # largest label id in CS is bicycle: 33
            BOUNDARY_ID = 34000
            binary_mask = (predBoundary > 0).astype(np.uint8)
            predBoundary = mask_to_boundary(binary_mask, args.dilation_ratio)
            boolpredBoundary = predBoundary != 0
            predBoundaryPixelCount = np.count_nonzero( boolpredBoundary )
        else:
            boolpredBoundary = None
        ...
        if args.iou_type == "boundary":
            predInstance["pixelCountBoundary"] = int(predBoundaryPixelCount)
        ...
        for (gtNum,gtInstance) in enumerate(gtInstancesOrig[labelName]):
            ...
            if args.iou_type == "boundary":
                boundary_intersection = np.count_nonzero( np.logical_and( gtBoundary == gtInstance["instID"] , boolpredBoundary) )
            else:
                # any positive number
                boundary_intersection = 1

            # If they intersect add them as matches to both dicts
            if (intersection > 0 and boundary_intersection > 0):
                ...
                if args.iou_type == "boundary":
                    gtCopy["boundary_intersection"]   = boundary_intersection
                    predCopy["boundary_intersection"] = boundary_intersection
                    # This is not in the `gtInstancesOrig`
                    gtCopy["pixelCountBoundary"]      = int((gtBoundary == gtInstance["instID"]).sum())
                ...
                for img in matches:
                    ...
                    for (gtI,gt) in enumerate(gtInstances):
                        ...
                        for pred in gt["matchedPred"]:
                            ...
                            if args.iou_type == "boundary":
                                overlap_boundary = float(pred["boundary_intersection"]) / (gt["pixelCountBoundary"]+pred["pixelCountBoundary"]-pred["boundary_intersection"])
                                overlap = min(overlap, overlap_boundary)
                    ...
                    for pred in predInstances:
                        ...
                        for gt in pred["matchedGt"]:
                            ...
                            if args.iou_type == "boundary":
                                overlap_boundary = float(gt["boundary_intersection"]) / (gt["pixelCountBoundary"]+pred["pixelCountBoundary"]-gt["boundary_intersection"])
                                overlap = min(overlap, overlap_boundary)
                            if overlap > overlapTh:
                                foundGt = True
                                break
